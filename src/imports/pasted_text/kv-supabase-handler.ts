...
app.post("/make-server-9aaa8c9c/reset-content", async (c) => {
  try {
    ...
    await kv.mdel(["logline", "synopsis", "synopsisThemes", "authorBio", "posterImage", "contactEmail", "copyright"]);
...
app.get("/make-server-9aaa8c9c/content", async (c) => {
  try {
    const keys = ["logline", "synopsis", "synopsisThemes", "authorBio", "posterImage", "contactEmail", "copyright"];
    ...
    const content = {
      ...
      copyright: values[6] || defaultContent.copyright
    };
